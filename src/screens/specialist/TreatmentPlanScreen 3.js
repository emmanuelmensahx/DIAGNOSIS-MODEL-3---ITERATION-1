...
export default function TreatmentPlanScreen({ navigation, route }) {
  ...
  const addMedication = () => {
    if (!newMedication.name.trim() || !newMedication.dosage.trim()) {
      Alert.alert('Error', 'Please provide medication name and dosage');
      return;
    }
  ...
  const addProcedure = () => {
    if (!newProcedure.name.trim()) {
      Alert.alert('Error', 'Please provide procedure name');
      return;
    }
  ...
  const saveTreatmentPlan = async () => {
    ...
    if (treatmentPlan.medications.length === 0 && treatmentPlan.procedures.length === 0) {
      Alert.alert('Error', 'Please add at least one medication or procedure');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        'The treatment plan has been saved successfully.',
        [
          {
            text: 'OK',
      ...
      <View style={styles.submitContainer}>
        ...
        >
          <MaterialCommunityIcons 
            name={isSubmitting ? "loading" : "content-save"} 
            size={24} 
            color="#FFFFFF" 
          />
